#!/usr/bin/env bash
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: scripts/backup.sh <database> <offsite-directory>" >&2
  exit 2
fi

database=$(realpath "$1")
destination=$(realpath -m "$2")
key_file=${STORYBLOCK_BACKUP_KEY_FILE:-}

if [[ ! -f "$database" ]]; then
  echo "Database does not exist: $database" >&2
  exit 1
fi
if [[ -z "$key_file" || ! -f "$key_file" ]]; then
  echo "STORYBLOCK_BACKUP_KEY_FILE must name a readable key file" >&2
  exit 1
fi

mkdir -p "$destination"
work=$(mktemp -d)
trap 'rm -rf "$work"' EXIT
stamp=$(date -u +%Y%m%dT%H%M%SZ)
plain="$work/storyblock-$stamp.db"
compressed="$plain.zst"
artifact="$destination/storyblock-$stamp.db.zst.enc"

sqlite3 "$database" ".timeout 5000" ".backup '$plain'"
integrity=$(sqlite3 "$plain" "PRAGMA integrity_check;")
if [[ "$integrity" != "ok" ]]; then
  echo "Backup integrity check failed: $integrity" >&2
  exit 1
fi
zstd -q -T0 -19 "$plain" -o "$compressed"
openssl enc -aes-256-cbc -pbkdf2 -salt \
  -in "$compressed" -out "$artifact" -pass "file:$key_file"
chmod 600 "$artifact"
sha256sum "$artifact" > "$artifact.sha256"
printf '{"artifact":"%s","created_at":"%s","integrity":"ok","sha256":"%s"}\n' \
  "$(basename "$artifact")" \
  "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  "$(cut -d' ' -f1 "$artifact.sha256")" > "$artifact.json"
echo "$artifact"
