...
const elements = Object.fromEntries([
  ...
  'meta-hash', 'meta-han-hash', 'method', 'path', 'token', 'send',
...
async function selectNovel(novelId) {
  ...
  try {
    ...
    if (state.selectedNovelId !== novelId) return;
    renderNovel(payload);
    show(elements['reader-loading'], false);
...
async function sendConsoleRequest() {
  ...
  const credential = elements.token.value.trim();
  ...
  try {
    const result = await fetch(elements.path.value.trim(), options);
    ...
    elements['response-status'].textContent = 'Network error';
