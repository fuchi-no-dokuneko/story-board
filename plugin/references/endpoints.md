...
- Errors: default `ApiProblem` (application/problem+json).
- Authorization note: Requires novel:read and binds the stored proposal to the credential novel; trusted reachable clients use the owner principal.
- Confidence: `confirmed-from-code`.
- Sources: `server/src/main/java/dev/storyblock/api/http/RewriteProposalReadController.java`, `server/src/main/java/dev/storyblock/api/http/ApiSecurityConfiguration.java`, `server/src/main/resources/openapi/storyblock-v1.yaml`, `server/src/test/java/dev/storyblock/api/http/OpenApiContractTest.java`.
